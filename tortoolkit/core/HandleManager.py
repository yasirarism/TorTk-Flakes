...
async def handle_leech_command(e):
    ...
    if not e.is_reply:
        await e.reply("⚠ Reply to a direct link, yt link, ig link, .torrent file, or magnet link.
🔰 *SUPPORT DIRECT LINK GENERATOR:*
 -Auto downloading file only for Host `Mediafire.com link` and `Zippyshare.com link`.")
    else:
        rclone = False
        tsp = time.time()
