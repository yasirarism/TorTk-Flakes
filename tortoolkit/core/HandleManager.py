...
async def handle_server_command(message):
    ...
    msg = (
            f"╭────────「 ⚡Statistik BOT 」\n<b>│\n├ ⏰ Bot berjalan:</b> {diff}\n"
            f"<b>├ 💾 Total ruang disk:</b> {totaldsk}\n"
            f"<b>├ 📀 Terpakai:</b> {useddsk}\n"
            f"<b>├ 💿 Tersisa:</b> {freedsk}\n"
            f"<b>├ 🔼 Data terunggah:</b> {upb}\n"
            f"<b>├ 🔽 Data terunduh:</b> {dlb}\n"
            f"<b>├ 🖥 CPU Frequency:</b> {freqcurrent}  Mhz\n"
            f"<b>├ 🖥 CPU:</b> {cpupercent}%\n"
            f"<b>├ 🎮 RAM:</b> {mempercent}%\n"
            f"<b>├ 🔥 Cores:</b> {cores} | <b>Logical:</b> {lcores}\n"
            f"│\n╰──「 ⚡Mod by <code>@yourtulloh</code> 」\n"
    )
...
async def about_me(message):
    ...
    msg = (
        ...
        "Mod by @yourtulloh Added Directlink generator for Mediafire and Zippyshare. UI litle bit changed.\n"
